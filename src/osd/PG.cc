...
void PG::scrub_compare_maps() 
{
  ...
  if (scrubber.end.is_max() ||
    ...
    map<hobject_t,ScrubMap::object>::iterator iter = scrubber.cleaned_meta_map.objects.end();
    --iter; // not empty, see if clause
    map<hobject_t,ScrubMap::object>::iterator begin = scrubber.cleaned_meta_map.objects.begin();
    while (iter != begin) {
      map<hobject_t,ScrubMap::object>::iterator next = iter--;
